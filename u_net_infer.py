...
def infer():
    ...
    #model_directory = config['model_directory'] + '120_100'
    #masks_directory = 'masks_' + str(config['num_epochs'])
    ...
    test_images_list = [os.path.join(
        config['test_images_path'], x) for x in test_list]
    ...
    net_arch.densenet121_encoder(test_images)
    ...
    tf.train.Saver().restore(ss, os.path.join(os.getcwd(), os.path.join(
        model_directory, config['model_file'] + '-' + str(298))))

    print('Inference Started.......................')

    for img_file in test_images_list:
        ti = time.time()
        labels_predicted = ss.run(labels_prediction, feed_dict={
                                  is_training: not(config['training'])})
        ti = time.time() - ti
        print('Time Taken for Inference : ' + str(ti))
        print('')

        labels_predicted_padded = np.transpose(
            labels_predicted, [1, 2, 0]).astype(np.uint8)
        labels_predicted_unpadded = labels_predicted_padded[:101, :101]
        cv2.imwrite(os.path.join(os.getcwd(), os.path.join(model_directory, os.path.join(
            masks_directory, 'mask_' + img_file.split('/')[-1]))), labels_predicted_unpadded)
