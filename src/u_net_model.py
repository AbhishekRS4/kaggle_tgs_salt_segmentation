...
class UNet:
    ...
    def densenet121_encoder(self, features):
        ...
        self.stage1 = tf.nn.max_pool(
            self.stage0, ksize=self._pool_kernel, strides=self._pool_strides,
        ...
        self.se1 = self._squeeze_excitation_block(self.stage2_dense, 256, name="se_block1_")
        self.stage2_transition = self._transition_block(self.se1, self._reduction_rate, name="pool2")

        # Stage 3
        self.stage3_dense = self._dense_block(self.stage2_transition, self._blocks[1], name="conv3")
        self.se2 = self._squeeze_excitation_block(self.stage3_dense, 512, name="se_block2_")
        self.stage3_transition = self._transition_block(self.se2, self._reduction_rate, name="pool3")

        # Stage 4
        self.stage4_dense = self._dense_block(self.stage3_transition, self._blocks[2], name="conv4")
        self.se3 = self._squeeze_excitation_block(self.stage4_dense, 1024, name="se_block3_")
        self.stage4_transition = self._transition_block(self.se3, self._reduction_rate, name="pool4")

        # Stage 5
        self.stage5_dense = self._dense_block(self.stage4_transition, self._blocks[3], name="conv5")
        self.se4 = self._squeeze_excitation_block(self.stage5_dense, 1024, name="se_block4_")
    ...
    def custom_u_net(self):
        ...
        self.concat1 = tf.concat([self.up1, self.se3], axis=self._feature_map_axis, name="concat1")
        ...
        self.se5 = self._squeeze_excitation_block(self.elu6_2, 1024, name="se_block5_")

        # decoder 2
        self.up2 = self._decoder_block(self.se5, self._num_kernels[3], 2)
        self.concat2 = tf.concat([self.up2, self.se2], axis=self._feature_map_axis, name="concat2")
        ...
        self.se6 = self._squeeze_excitation_block(self.elu7_2, 512, name="se_block6_")

        # decoder 3
        self.up3 = self._decoder_block(self.se6, self._num_kernels[2], 3)
        self.concat3 = tf.concat([self.up3, self.se1], axis=self._feature_map_axis, name="concat3")
        ...
        self.se7 = self._squeeze_excitation_block(self.elu8_2, 256, name="se_block7_")

        # decoder 4
        self.up4 = self._decoder_block(self.se7, self._num_kernels[0], 4)
        self.concat4 = tf.concat([self.up4, self.stage0], axis=self._feature_map_axis, name="concat4")
        ...
        self.se8 = self._squeeze_excitation_block(self.elu9_2, 64, name="se_block8_")
    ...
    def _avg_pool(self, input_layer, pool_size=[2, 2], strides=[2, 2], name="avg_pool"):
        avg_pool_layer = tf.layers.average_pooling2d(input_layer, pool_size=pool_size,
            strides=strides, padding=self._padding, data_format=self._data_format, name=name
        )
    ...
    def _conv_layer(self, input_layer, name, strides=[1, 1, 1, 1]):
        ...
        W_init_value = np.array(weights_hierarchy["kernel:0"]).astype(np.float32)
        W = tf.get_variable(name=name + "_w", initializer=W_init_value, dtype=tf.float32)
    ...
    #--------------------------------------#
    # trainable densenet decoder functions #
    #--------------------------------------#
    #--------------------------------#
    ...
    def _normal_conv_block(self, input_layer, num_kernels, kernel_size, padding, num_1, num_2):
        ...
        _bn1 = self._get_batchnorm_layer(_conv1, name="bn" + str(num_1) + "_" + str(num_2))
        _elu1 = self._get_elu_activation(_bn1, name="elu" + str(num_1) + "_" + str(num_2))
    ...
    def _decoder_block(self, input_layer, num_kernels, num_1):
        _conv_tr = self._get_conv2d_transpose_layer(input_layer, num_kernels,
            self._conv_tr_kernel_size, self._conv_tr_strides, self._padding, name="up_tr_conv" + str(num_1)
        )
    ...
    def _squeeze_excitation_block(self, input_layer, input_channels, ratio=16, name="se_block"):
        _avg_pool = tf.reduce_mean(input_layer, axis=self._avg_pool_axes,
            name=name + "avg_pool", keepdims=True
        )
        _conv1 = self._get_conv2d_layer(_avg_pool, int(input_channels / ratio),
            self._conv_reduction_kernel_size, self._conv_strides, self._padding, name=name + "squeeze"
        )
    ...
    def _get_conv2d_layer(self, input_layer, num_filters, kernel_size, strides, padding, name="conv", dilation_rate=[1, 1], use_bias=False):
        conv_2d_layer = tf.layers.conv2d(inputs=input_layer, filters=num_filters, kernel_size=kernel_size,
            strides=strides, padding=padding, data_format=self._data_format, dilation_rate=dilation_rate,
            kernel_initializer=self._initializer, use_bias=use_bias, name=name
        )
    ...
    def _get_elu_activation(self, input_layer, name="elu"):
        elu_layer = tf.nn.elu(input_layer, name=name)
        return elu_layer
    ...
    def _get_sigmoid_activation(self, input_layer, name="sigmoid"):
        sigmoid_layer = tf.nn.sigmoid(input_layer, name=name)
    ...
    def _get_batchnorm_layer(self, input_layer, name="bn"):
        bn_layer = tf.layers.batch_normalization(input_layer, axis=self._feature_map_axis, training=self._training, name=name)
        return bn_layer
    ...
    def _get_conv2d_transpose_layer(self, input_features, num_filters, kernel_size, strides, padding, use_bias=False, name="conv_tr"):
        conv_2d_tr_layer = tf.layers.conv2d_transpose(inputs=input_features, filters=num_filters,
            kernel_size=kernel_size, strides=strides, padding=padding, data_format=self._data_format,
            kernel_initializer=self._initializer, use_bias=use_bias, name=name
        )
