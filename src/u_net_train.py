...
def dice_loss(ground_truth, predicted_logits, axis=-1, smooth=1e-5, name="mean_dice_loss"):
    predicted_probs = get_softmax_layer(input_tensor=predicted_logits, axis=axis)
    predicted_class = tf.round(predicted_probs)
    intersection = tf.reduce_sum(tf.multiply(ground_truth, predicted_class), axis=[1, 2, 3])
    union = tf.reduce_sum(ground_truth, axis=[1, 2, 3]) + tf.reduce_sum(predicted_class, axis=[1, 2, 3])
...
def save_model(session, model_directory, model_file, epoch):
    ...
    saver.save(session, os.path.join(os.getcwd(),
        os.path.join(model_directory, model_file)), global_step=(epoch + 1))
...
def batch_train():
    ...
    custom_u_net.densenet121_encoder(features)
    custom_u_net.custom_u_net()
    logits = custom_u_net.logits
    ...
    for epoch in range(num_epochs):
        ...
        for batch_id in range(num_train_batches):
            _, _, loss_per_batch = ss.run(
                ...
                feed_dict={training_pl: bool(config["training"]), global_step_pl: epoch}
            )
            temp_train_loss_per_epoch += loss_per_batch

        for batch_id in range(num_valid_batches):
            loss_per_batch = ss.run(loss, feed_dict={training_pl: not(config["training"])})
        ...
        print(f"Epoch : {epoch + 1} / {num_epochs}, time taken : {ti} sec.")
        print(f"Avg. training loss : {temp_train_loss_per_epoch / num_train_batches:.4f}, \
            Avg. validation loss : {temp_valid_loss_per_epoch / num_valid_batches:.4f}")
